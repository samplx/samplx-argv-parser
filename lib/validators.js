...
module.exports = {
    ...
    positiveInteger : function (errMsg) {
        return function (opt) {
            if (!opt.value || (opt.value == 'Infinity')) return;
            var value = parseInt(opt.value, 10);
            if (!/^\d+$/.test(opt.value) || isNaN(value) || (value < 0)) {
                throw new Error(interpolate(errMsg || "${1}: ${2} is not a positive integer.", 
                        [ opt.signature, opt.value ]
                ));
            }
        };
    ...
    isURL: function (errMsg) {
        return function (opt) {
            if (!opt.value || urlPattern.test(opt.value)) return;
            throw new Error(interpolate(
                errMsg || "${1}: ${2} is not a valid URL.",
                    [ opt.signature, opt.value ]
            ));
        };
    },
