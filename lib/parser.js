...
const opd = require("./operand");
const OPTIONS_END_DELIMITER = "--";
...
function tokenize(args, options) {
    ...
    const expanded = expandShorthands(args, options);
    const result = expanded.reduce(function (tokenized, arg) {
...
function parse(options, args, done) {
        ...
        when.all(promises).then(function () {
            ...
            for (prop in results) {
                try {
                    ...
                    const message = results[prop].signature + ": " + e.message;
                    return done(message);
                }
            }
            done(null, results);
    ...
    function value(opt, arg) {
        ...
        const val = valueFromEqualsSign(arg);
        if (val) { return val; }
    ...
    function handleNext(arg) {
        return options.some(function (option) {
            const data = resultFor(option);
            if (option.recognizes(arg, data) && option.handle) {
                const val = value(option, arg);
